                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade" id="tab-form" role="tabpanel" aria-labelledby="tab-form">
                            <form data-plugin="parsley" data-option="{}" id="form">
                                ...
                                <div class="form-group">
                                    <label>Tanggal Sampai Bis</label>
                                ...
                                <div class="form-group">
                                    <label>Kuota Public</label>
                                    <input type="number" class="form-control form-control-md" name="quota_public" id="quota_public" placeholder="Kuota Public" required autocomplete="off">
                                </div>
                                <div class="form-group">
                                    <label>Kuota Paguyuban</label>
                                    <input type="number" class="form-control form-control-md" name="quota_paguyuban" id="quota_paguyuban" placeholder="Kuota Paguyuban" required autocomplete="off">
                                </div>
                                <div class="form-group">
                                    <label>Total Kuota</label>
                                    <input type="number" class="form-control form-control-md" name="quota_max" id="quota_max" placeholder="Kuota Maksimal" required autocomplete="off" readonly>
                                </div>
                                <div class="form-group">
                                    <label>Arus</label><br>
...
<script type="text/javascript">
    ...
    $(document).on("keyup", "#quota_public", function() {
        var quota_public = $('#quota_public').val().length != 0 ? $('#quota_public').val() : 0;
        var quota_paguyuban = $("#quota_paguyuban").val().length != 0 ? $("#quota_paguyuban").val() : 0 ;

        $("#quota_max").val(parseInt(quota_public) + parseInt(quota_paguyuban));
    });

    $(document).on("keyup", "#quota_paguyuban", function() {
        var quota_public = $('#quota_public').val().length != 0 ? $('#quota_public').val() : 0;
        var quota_paguyuban = $("#quota_paguyuban").val().length != 0 ? $("#quota_paguyuban").val() : 0 ;
        ...
        let columns = [{
            ...
            },
            {
                ...
                render: function(a, type, data, index) {
                    ...
                    if (data.open == "0") {
                        button += `<button class="btn btn-sm btn-outline-danger closed" title="Closed">
                                            Closed
                                        </button>`
                    } else {
                        button += `<button class="btn btn-sm btn-outline-success open" title="open">
                                            Open
                                        </button>`
                    }
