...
class kmtAction extends BaseController {
    private $kmtModel;
    ...
    public function pdf() {
        ...
        $data['data_excel'] = $this->kmtModel->export_view($url_export, $filter);

        $html = view("kmt/Views/$url", $data);
        $mpdf = new \Mpdf\Mpdf();
        if (isset($_GET['o'])) {
            if ($_GET['o'] == 'l') {
                $mpdf->AddPage('L');
    ...
    public function excel() {
        ...
        echo view("kmt/Views/$url", $data);
    }

    public function lapkmt_load()
    {
