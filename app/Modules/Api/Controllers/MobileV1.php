...
class MobileV1 extends BaseController
{
    ...
    public function authMobileUser(){
        ...
        if(empty($query)){
            ...
            if($query->user_mobile_role == '2'){
                ...
                b.user_mobile_phone,b.user_mobile_type,b.user_mobile_photo,b.user_mobile_uid,
                b.user_mobile_fcm,user_mobile_jitsi,b.user_mobile_rating,b.user_mobile_version,b.user_mobile_role
                from m_user_web a
                left join m_user_mobile b on a.`user_mobile_id`=b.id where a.user_mobile_id = '".$query->id."'")->getRow();
                return $this->response->setJSON(res_success_custom3(1, 'Success', $userPetugas,'user_role_code',$tipeUser->user_mobile_role_code,'user_role_name',$tipeUser->user_mobile_role_name,'is_otp',$is_otp));
            }else if($query->user_mobile_role == '3'){
                $this->apiModel->base_update($data, "m_user_mobile", ["id" => $query->id]);
                return $this->response->setJSON(res_success_custom3(1, 'Success', $query,'user_role_code',$tipeUser->user_mobile_role_code,'user_role_name',$tipeUser->user_mobile_role_name,'is_otp',$is_otp));
            }
            else{
    ...
    //     $res_where = [];
    //     !isset($data['noken']) ?: $res_where['noken'] = addslashes($data['noken']);
    //     !isset($data['no_kps']) ?: $res_where['no_kps'] = addslashes($data['no_kps']);

    //     if(empty($res_where)) {
    //         return $this->response->setJSON(res_notfound(1, "please select data noken atau no_kps"));
    //     }

    //     $id = res_where($res_where, 'where');

    //     $query = $this->db->query("select id,perusahaan_id,jenis_pelayanan,kode_kendaraan,noken,no_uji,tgl_exp_uji,no_kps,tgl_exp_kps,no_srut,CONCAT( LEFT( no_rangka, LENGTH( no_rangka ) -3 ) ,  'xxx' ) as no_rangka,CONCAT( LEFT( no_mesin, LENGTH( no_mesin ) -3 ) ,  'xxx' ) as no_mesin,merek,tahun,seat,barang,kode_trayek,nama_trayek,rute,etl_date,link_pdf
    //     from m_spionam " . $id . " order by tgl_exp_kps desc");
        ...
        if($jadwalMudik->jadwal_type == 1){
            ...
            try{
                ...
                if($dataBuyer->billing_status_payment == '1' || $dataBuyer->billing_expired_date  >= date('Y-m-d H:i:s')){ //|| $dataBuyer->billing_expired_date  >= date('Y-m-d H:i:s')
                    ...
                    $response = [
                        'status' => 3,
                        'message' => 'Anda sudah melakukan pemesanan / NIK sudah pernah mendaftar namun sampai batas akhir tidak melakukan validasi'
                    ];
                    return $this->response->setJSON($response);
                }else{
                try{
    ...
    public function transactionMotis(){
        ...
        $jadwalMudik = $this->db->query("SELECT a.billing_verif_expired_date,DATE_ADD(curdate(),INTERVAL 7 DAY) as date_max_expired from t_billing_mudik a 
                                        LEFT JOIN t_jadwal_mudik b on a.`billing_jadwal_id`=b.id
            ...
            foreach ($jadwal_arr as $value) {
                ...
                where a.id='".$value."'")->getRow();
                    ...
                    if($cekKouta->jadwal_type == 1){
                        $jadwalVerif = $jadwalMudik->date_max_expired;
    ...
    public function checkManifestNik(){
        $data = $this->request->getPost();
        $nik = $data['nik'];
        ...
        CASE
            WHEN  b.billing_status_verif = 1 THEN '1'
            WHEN b.billing_status_verif = 0 THEN '3'
            ELSE '2'
        END as billing_status_code
        ...
        left join m_lokabkota g on e.`idkabkota`=g.id
        left join m_lokabkota h on f.idkabkota=h.id
        where a.transaction_nik='".addslashes($nik)."' and b.billing_status_payment=1")->getRow();

        if(!empty($query->data_nik)){
            if($query->billing_status_code == 1){
                $response = [
                    'status_code' => 1,
                    'status_message' => 'Success' ,
                    'status_booking' => $query->billing_status_code,
                    'status_booking_message' => 'verified'
                ];
        
                return $this->response->setJSON($response);
    
            }else if ($qyert->billing_status_code == 2){
                $response = [
                    'status_code' => 1,
                    'status_message' => 'Success' ,
                    'status_booking' => $query->billing_status_code,
                    'status_booking_message' => 'not verify'
                ];
                ...
                $response = [
                    ...
                    'status_message' => 'Success' ,
                    'status_booking' => $query->billing_status_code,
                    'status_booking_message' => 'waiting for verification'
                ];
            ...
            $response = [
                ...
                'status_booking' => 0,
                'status_booking_message' => 'not registered'
            ];
