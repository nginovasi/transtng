...
namespace App\Modules\Pomudik\Models;

use App\Core\BaseModel;
use LDAP\Result;
use Mpdf\Tag\Q;

class PomudikModel extends BaseModel
{
	// get all facilities
		...
		for ($i = 0; $i < count($data['paguyuban_tempat_duduk']); $i++) {
			...
			if (!$paguyubanDetail) {
				...
				if (!empty($cekEmail)) {
					...
					} else if($billings && unwrap_null(@$data['paguyuban_no_wa'][$i], "-") != "GENERAL ORDER" && unwrap_null(@$data['paguyuban_no_kk'][$i], "-") != "GENERAL ORDER") {
						// update qty billing
				...
				if (!empty($cekEmail)) {
					...
					// paguyuban not delete by email because email make select 2
			...
			if(@$data['email'][$i] != null) {
				if($data['billing_id'][$i] == "") {
					...
					$paguyuban_motis = [
						...
						"motis_verif_expired_date" => date("Y-m-d H:i:s"),
						"motis_status_verif" => 1,
						"motis_date_verif" => date("Y-m-d H:i:s"),
						"motis_armada_id" => $data['armada_mudik'][$i]
					];
					...
					$motis = [
						...
						"nama_pemilik_kendaraan" => $data['nama_pemilik_kendaraan'][$i],
					...
					if($jadwalMotisBalik) {
						$paguyuban_motisbalik = [
							...
							"motis_status_verif" => 1,
							"motis_date_verif" => date("Y-m-d H:i:s"),
							"motis_armada_id" => $data['armada_balik'][$i]
						];
