...
<script type="text/javascript">
    ...
    const url_pdf = '<?= base_url() . "/" . uri_segment(0) . "/pdf/exportTrxPerPetugasHarian" . "" ?>';
    const url_pdf_detail = '<?= base_url() . "/" . uri_segment(0) . "/pdf/exportTrxPerPetugasDetailHarian" . "" ?>';
    ...
    $('#date').on('change', function() {
        ...
        $.ajax({
            method: "post",
            dataType : "json",
            url: url_ajax + "/getTrxPerPetugasHarian",
            data:{
                <?= csrf_token() ?>: "<?= csrf_hash() ?>",
                "date": date,
            ...
            success : function (rs) {
                ...
                if(rs.success){
                    ...
                    $(".tab-content-title").text(`REKAP LAPORAN TRANSAKSI PER PETUGAS PERIODE ${explodeDateHarian[2]} ${getMonth(explodeDateHarian[1])} ${explodeDateHarian[0]}`)
                            ...
                            <table class="table table-striped table-bordered table-hover" id="">
                                <thead>
                                    <tr class="border-dark" style="border:1px solid #555255">
                                        ...
                                        <th class="text-center border-dark" style="border:1px solid #555255">Username</th>
                                        <th class="text-center border-dark" style="border:1px solid #555255">Shift</th>
                                        <th class="text-center border-dark" style="border:1px solid #555255">Bis</th>
                    ...
                    $.each(rs.data.result, function(i, val) {
                        result += `
                            <tr class="border-dark" style="border:1px solid #555255">
                                ...
                                <td class="text-center border-dark" style="border:1px solid #555255">${val.user_web_username}</td>
                                <td class="text-center border-dark" style="border:1px solid #555255">
                                    <form class="pdf_detail_pta_export" action="<?= base_url()?>/eksekutif/pdf/exportTrxPerPetugasDetailHarian" target="_blank" method="get" id="form${i + 1}">
                                        
                                        <a href="#" class="detail_pta text-primary font-weight-bolder d-block font-size-lg" style="margin-top: 5px">(Shift ${val.shift})</a>
                                        <input class="form-control" type="hidden" name="d_username" value="${val.user_web_username}">
                                        <input class="form-control" type="hidden" name="d_tanggal" value="${date}">
                                        <input class="form-control" type="hidden" name="d_user_web_username" value="${val.user_web_username}">
                                        <input class="form-control" type="hidden" name="d_shift" value="${val.shift}">
                                        <input class="form-control" type="hidden" name="d_kode_bis" value="${val.kode_bis}">
                                        <input class="form-control" type="hidden" name="d_device_id" value="${val.device_id}">
                                        <input class="form-control" type="hidden" name="d_jalur" value="${val.jalur}">
                                    </form>
                                </td>
                                <td class="text-center border-dark" style="border:1px solid #555255">${val.kode_bis}</td>
                    ...
                    result += `</tbody>
                                <tfoot>
                                    <td colspan="6" class="text-right font-weight-bold border-dark" style="border:1px solid #555255">Total</td>
    ...
    $(document).on('click', '.detail_pta', function(e) {
        e.preventDefault();
        Swal.fire({
            title: "Export PDF",
            icon : "warning",
