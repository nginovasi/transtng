                        ...
                        <div class="tab-pane fade active show" id="tab-form" role="tabpanel" aria-labelledby="tab-form">
                            <form data-plugin="parsley" data-option="{}" id="form">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="user_type_name" name="user_type_name" required autocomplete="off" placeholder="Tentukan nama jenis user">
                                </div>
                                <div class="form-group">
                                    <label>Kode Ldap</label>
                                    <input type="text" class="form-control" id="user_type_code" name="user_type_code" required autocomplete="off" placeholder="Tentukan kode jenis user untuk LDAP">
                                </div>
...
<script type="text/javascript">
                ...
                if(result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(result){
                            Swal.close();
                            if(result.success){
                                Swal.fire('Sukses','Berhasil menyimpan jenis user', 'success');
                                $('#form').trigger("reset");
                                table.ajax.reload();
                            }else{
                ...
                if(result.value) {
                    $.ajax({
                        ...
                        success: function(result){
                            Swal.close();
                            if(result.success){
                                Swal.fire('Sukses','Berhasil menghapus jenis user', 'success');
                                table.ajax.reload();
                            }else{
                                Swal.fire('Error',result.message, 'error');
                            }
    ...
    function datatableColumn(){
        let columns = [
                ...
                {data: "user_type_code", orderable: true},
                {
